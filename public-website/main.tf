...
locals {
  domain = var.subdomain != "" ? "${var.subdomain}.${lookup(var.stage_domains, "domain", "unknown-domain")}" : lookup(var.stage_domains, "domain", "unknown-domain")
}
...
resource "aws_cloudfront_distribution" "distribution" {
  ...
  viewer_certificate {
    acm_certificate_arn      = lookup(var.stage_domain, "certificate_arn", "unknown-certificate-arn")
...
resource "aws_route53_record" "api_record" {
  ...
  zone_id = lookup(var.stage_domains, "zone_id", "unknown-zone-id")
  name    = local.domain
  type    = "CNAME"
  ttl     = "600"
